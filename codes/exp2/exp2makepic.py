...
def generate_image_with_boxes_and_compare(noise, colormap, output_filepath, result_filepath, box_count=8):
    ...
    with open(result_filepath, "a") as result_file:  # "a" 模式为追加写入
        ...
        for i in range(box_count):
            ...
            # 比较两个平均值，记录较大的方框颜色
            if red_box_avg > blue_box_avg:
                larger_box_color = "red"
            else:
                larger_box_color = "blue"
            # 写入结果文件
            # 修改文件名，去掉 "ScalarField_WithBoxes_Noise_" 前缀
            image_basename = os.path.basename(output_filepath).replace("ScalarField_WithBoxes_Noise_", "").replace(".png", f"_{i+1}")
            result_file.write(f"{image_basename}, Red Box Avg: {red_box_avg:.4f}, Blue Box Avg: {blue_box_avg:.4f}, Larger Box: {larger_box_color}\n")
            print(f"第 {i+1} 个图像 - 红框平均值: {red_box_avg}, 蓝框平均值: {blue_box_avg}, 较大方框: {larger_box_color}")
            # 绘制图像并添加方框
            ...
            plt.margins(0, 0)
            # 绘制红色方框（区域 1）
            rect1 = plt.Rectangle((x1, y1), 100, 100, edgecolor='red', facecolor='none', linewidth=2)
            plt.gca().add_patch(rect1)
            # 绘制蓝色方框（区域 2）
            rect2 = plt.Rectangle((x2, y2), 100, 100, edgecolor='blue', facecolor='none', linewidth=2)
...
def main():
    ...
    output_dir = f"E:\桌面\Final project\color\exp2\{colormap}"  # 输出图像文件夹
